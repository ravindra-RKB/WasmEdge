...
""".format(
    ...
    CONST_env_fish = """#!/usr/bin/env fish
# wasmedge shell setup
if not contains "{0}/bin" $PATH
    set -x PATH "{0}/bin" $PATH
end
if not contains "{0}/{1}" ${2}
    set -x {2} "{0}/{1}" ${2}
end
if not contains "{0}/{1}" $LIBRARY_PATH
    set -x LIBRARY_PATH "{0}/{1}" $LIBRARY_PATH
end
if not contains "{0}/include" $C_INCLUDE_PATH
    set -x C_INCLUDE_PATH "{0}/include" $C_INCLUDE_PATH
end
if not contains "{0}/include" $CPLUS_INCLUDE_PATH
    set -x CPLUS_INCLUDE_PATH "{0}/include" $CPLUS_INCLUDE_PATH
end
if not set -q WASMEDGE_LIB_DIR
    set -x WASMEDGE_LIB_DIR "{0}/{1}"
end
# Please do not edit comments below this for uninstallation purpose
""".format(args.path, CONST_lib_dir, compat.ld_library_path)
    ...
    CONST_env_fish_path = join(args.path, "env.fish")
    mode = "w+" if not exists(CONST_env_path) else "w"
    mode_fish = "w+" if not exists(CONST_env_fish_path) else "w"
    ...
    with opened_w_error(CONST_env_fish_path, mode_fish) as env_fish:
        if env_fish is not None:
            env_fish.write(CONST_env_fish)
        else:
            logging.error("Not able to write to env.fish file")
...
def shell_configure(args, compat):
    ...
    if ("bash" in SHELL) or ("zsh" in SHELL):
        ...
        fish_dir = join(HOME, ".config", "fish")
        if not exists(fish_dir):
            try:
                os.makedirs(fish_dir)
            except:
                pass
        CONST_shell_config = join(fish_dir, "config.fish")
        if not exists(CONST_shell_config):
            try:
                ...
                pass
        write_shell = False
        with opened_w_error(CONST_shell_config, "r") as shell_config_file:
            if shell_config_file is not None:
                if source_string_fish not in shell_config_file.read():
                    write_shell = True
            else:
                write_shell = True
                
        if write_shell:
            with opened_w_error(CONST_shell_config, "a") as shell_config_file:
                if shell_config_file is not None:
                    shell_config_file.write(source_string_fish)
